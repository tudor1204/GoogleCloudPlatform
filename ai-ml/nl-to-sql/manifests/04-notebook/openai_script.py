...
llm = ChatOpenAI(
    openai_api_base=os.environ.get("LLM_ENDPOINT"),
    temperature=0.5, 
...
sql_prompt_template = ChatPromptTemplate.from_messages(
    [
        ...
        {db_schema}
        Please prepare and return only the SQL command, based on the user query, without any formatting or newlines. The answer must contain only valid SQL command.
        """),
        ("human", "{query}"),
    ]
)
...
def handle_query(query):
    sql_prompt_value=sql_prompt_template.format_messages(db_schema=db_schema_formatted, query=query)
    sql_query=llm.invoke(sql_prompt_value).content
    print(sql_query)
    postgres_reply=postgres_query(sql_query)
    if postgres_reply == False:
        return "Try another query"
    print(postgres_reply)
    final_prompt_value=final_prompt_template.format_messages(db_schema=db_schema_formatted, query=query, postgres_reply=postgres_reply)
    return llm.invoke(final_prompt_value).content
