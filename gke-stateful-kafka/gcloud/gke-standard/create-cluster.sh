...
export GKE_NODE_POOL_ZONES="$REGION-a,$REGION-b,$REGION-c"
export GKE_NODE_POOL_IMAGE_TYPE=COS_CONTAINERD
export GKE_OAUTH_SCOPES=https://www.googleapis.com/auth/cloud-platform
export GKE_RELEASE_CHANNEL=regular
#export GKE_CLUSTER_VERSION_ARGS="--cluster-version=1.24"
...
export DR_GKE_NODE_POOL_ZONES="$DR_REGION-a,$DR_REGION-b,$DR_REGION-c"
export DR_GKE_NODE_POOL_IMAGE_TYPE=COS_CONTAINERD
...
create_cluster ()
{
  ...
  gcloud beta container clusters create $GKE_CLUSTER_NAME \
      ...
      --addons HorizontalPodAutoscaling,HttpLoadBalancing,GcePersistentDiskCsiDriver,NetworkPolicy,BackupRestore \
      --enable-autoupgrade \
      --enable-autorepair \
      --max-surge-upgrade 1 \
      ...
      --min-memory 240 \
      --max-memory 288 \
      --autoprovisioning-image-type $GKE_NODE_POOL_IMAGE_TYPE \
      ...
      --autoprovisioning-scopes $GKE_OAUTH_SCOPES \
  ...
  export GKE_NOOD_POOL_MACHINE_TYPE=e2-standard-4
  check_cluster_status "Timeout waiting for Cluster $GKE_CLUSTER_NAME to be in RUNNING state. Could not create node pool $GKE_NODE_POOL_NAME"
  echo Creating node pool $GKE_NODE_POOL_NAME for cluster $GKE_CLUSTER_NAME...
  gcloud beta container node-pools create $GKE_NODE_POOL_NAME \
      --project $PROJECT_ID \
      --cluster $GKE_CLUSTER_NAME \
      --region=$REGION \
      --machine-type $GKE_NOOD_POOL_MACHINE_TYPE \
      --image-type $GKE_NODE_POOL_IMAGE_TYPE \
      --node-labels cluster_name=$GKE_CLUSTER_NAME,node_pool=$GKE_NODE_POOL_NAME \
      --metadata disable-legacy-endpoints=true,cluster_name=$GKE_CLUSTER_NAME,node_pool=$GKE_NODE_POOL_NAME \
      --service-account $SERVICE_ACCOUNT_NAME@$PROJECT_ID.iam.gserviceaccount.com \
      --num-nodes 1 \
      --enable-autoscaling \
      --min-nodes 1 \
      ...
      --max-surge-upgrade 1 \
      --max-unavailable-upgrade 0 \
      --pod-ipv4-range ip-range-pods-$REGION \
      --tags "gke-$GKE_CLUSTER_NAME","gke-$GKE_CLUSTER_NAME-$GKE_NODE_POOL_NAME" \
      --node-locations=$GKE_NODE_POOL_ZONES

  # Delete default node pool
    check_cluster_status "Timeout waiting for Cluster $GKE_CLUSTER_NAME to be in RUNNING state. Could not delete default node pool of cluster $GKE_CLUSTER_NAME"
    echo Deleting default node pool...
    gcloud -q container node-pools delete default-pool --cluster=$GKE_CLUSTER_NAME \
        --project $PROJECT_ID \
        --region=$REGION

  # https://cloud.google.com/sdk/gcloud/reference/container/node-pools/create
  export GKE_NODE_POOL_NAME=pool-kafka
  export APP_COMPONENT=kafka-broker
  export GKE_NOOD_POOL_MACHINE_TYPE=e2-standard-8
  check_cluster_status "Timeout waiting for Cluster $GKE_CLUSTER_NAME to be in RUNNING state. Could not create node pool $GKE_NODE_POOL_NAME"
  echo Creating node pool $GKE_NODE_POOL_NAME for cluster $GKE_CLUSTER_NAME...
  gcloud beta container node-pools create $GKE_NODE_POOL_NAME \
      --project $PROJECT_ID \
      ...
      --machine-type $GKE_NOOD_POOL_MACHINE_TYPE \
      --image-type $GKE_NODE_POOL_IMAGE_TYPE \
      --node-labels cluster_name=$GKE_CLUSTER_NAME,node_pool=$GKE_NODE_POOL_NAME,app.stateful/component=$APP_COMPONENT \
      --metadata disable-legacy-endpoints=true,cluster_name=$GKE_CLUSTER_NAME,node_pool=$GKE_NODE_POOL_NAME \
      --node-taints app.stateful/component=$APP_COMPONENT:NoSchedule \
      --service-account $SERVICE_ACCOUNT_NAME@$PROJECT_ID.iam.gserviceaccount.com \
      --num-nodes 1 \
  ...
  # https://cloud.google.com/sdk/gcloud/reference/container/node-pools/create
  export GKE_NODE_POOL_NAME=pool-zookeeper
  export APP_COMPONENT=zookeeper
  export GKE_NOOD_POOL_MACHINE_TYPE=e2-standard-8
  check_cluster_status "Timeout waiting for Cluster $GKE_CLUSTER_NAME to be in RUNNING state. Could not create node pool $GKE_NODE_POOL_NAME"
  echo Creating node pool $GKE_NODE_POOL_NAME for cluster $GKE_CLUSTER_NAME...
  gcloud beta container node-pools create $GKE_NODE_POOL_NAME \
...
then
  ...
  then
    ...
    export GKE_NODE_POOL_ZONES=$DR_GKE_NODE_POOL_ZONES
    export GKE_NODE_POOL_IMAGE_TYPE=$DR_GKE_NODE_POOL_IMAGE_TYPE
