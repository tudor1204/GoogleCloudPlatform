...
export VPC_NAME=vpc-gke-postgresql
export SUBNET_NAME=snet-gke-postgresql-$REGION
...
export SERVICE_ACCOUNT_NAME=gke-postgresql-sa
...
export GKE_CLUSTER_NAME=cluster-db1
...
export DR_SUBNET_NAME=snet-gke-postgresql-$DR_REGION
...
export DR_GKE_CLUSTER_NAME=cluster-db2
...
create_cluster ()
{
  ...
  export GKE_NODE_MACHINE_TYPE=e2-standard-4
  ...
  # https://cloud.google.com/sdk/gcloud/reference/container/node-pools/create
  export GKE_NODE_POOL_NAME=pool-db
  export APP_COMPONENT=postgresql
  export GKE_NODE_MACHINE_TYPE=e2-standard-8
  check_cluster_status "Timeout waiting for Cluster $GKE_CLUSTER_NAME to be in RUNNING state. Could not create node pool $GKE_NODE_POOL_NAME"
  echo Creating node pool $GKE_NODE_POOL_NAME for cluster $GKE_CLUSTER_NAME...
  gcloud beta container node-pools create $GKE_NODE_POOL_NAME \
      --project $PROJECT_ID \
